...
export default function MissionPage() {
  ...
  const [viewMode, setViewMode] = useState<"configure" | "outcome">(
      ...
      {viewMode === "configure" ? (
        <div className="animate-fade-in pb-24 space-y-6" data-testid="mission-configure-view">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            ...
            {/* Quick run trigger in header: Unified Primary Action Button 1 */}
            <div className="flex items-center gap-2">
          ...
          {/* Parameter Stepper: Unified Primary Action Button 2 */}
          <ParameterStepper
          ...
          {/* Unified Primary Action Button 3: Sticky Bottom Action Bar */}
          <div
            data-testid="mission-sticky-bottom-bar"
            className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-surface/95 backdrop-blur-md px-4 py-3 shadow-lg md:left-64"
          >
            <div className="flex flex-wrap items-center justify-between gap-3 max-w-7xl mx-auto">
              <div className="flex flex-wrap items-center gap-2 text-xs">
                <span className="font-semibold text-text-muted hidden sm:inline">Configuration:</span>
                <span className="rounded-md border border-border bg-surface2 px-2.5 py-1 font-bold text-navy-900">
                  {mission.name}
                </span>
                <span className="text-text-muted hidden sm:inline">·</span>
                <span className="rounded-md border border-border bg-surface2 px-2.5 py-1 font-semibold text-navy-800">
                  {vessel.name} ({vessel.iceClass})
                </span>
                <span className="text-text-muted hidden sm:inline">·</span>
                <span className="rounded-md border border-border bg-surface2 px-2.5 py-1 font-mono text-[11px] font-bold text-blue-600">
                  {forecastHorizon * 24}h Window
                </span>
                <span className="text-text-muted hidden sm:inline">·</span>
                <span className="rounded-md border border-border bg-surface2 px-2.5 py-1 font-mono text-[11px] font-bold uppercase text-navy-900">
                  {preference}
                </span>
              </div>
              ...
              <button
                ...
                className={cn(
                  "flex items-center gap-2 rounded-xl px-5 py-2.5 text-xs sm:text-sm font-bold text-white transition-all shadow-md cursor-pointer",
                  simulationStatus === "running"
                    ? "bg-navy-700 opacity-80 cursor-not-allowed"
                    : "bg-navy-900 hover:bg-navy-800 hover:scale-[1.01]"
                )}
              >
                {simulationStatus === "running" ? (
                  ...
                  <>
                    <Zap size={15} className="fill-current text-amber-400" />
                    Generate Plan & Run Simulation
                  </>
                )}
              </button>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-border bg-surface p-4 shadow-sm">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs">
                ...
                <span className="text-text-muted">·</span>
                <span className="rounded bg-surface2 border border-border px-1.5 py-0.5 font-mono text-[10px] font-bold uppercase text-navy-900">
                  {preference}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={simulationStatus === "running"}
                className={cn(
                  "flex items-center gap-2 rounded-lg px-3.5 py-2 text-xs font-bold text-white transition-colors cursor-pointer",
                  simulationStatus === "running"
                    ? "bg-navy-700 opacity-80 cursor-not-allowed"
